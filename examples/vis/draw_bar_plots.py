...
def draw_plot(results_json_path,datalist,dataset_name):
    ...
    for data_name in results.keys():
        ...
        success_kcf_hog_all = np.array(get_thresh_success_pair(gts, kcf_hog_preds)[1])[69]
        success_ldes_all = np.array(get_thresh_success_pair(gts, ldes_preds)[1])[69]
        success_strcf_all = np.array(get_thresh_success_pair(gts, strcf_preds)[1])[69]
        success_csrdcf_all = np.array(get_thresh_success_pair(gts, csrdcf_preds)[1])[69]
        success_dimp50_all = np.array(get_thresh_success_pair(gts, dimp50_preds)[1])[69]
        success_prdimp50_all = np.array(get_thresh_success_pair(gts, prdimp50_preds)[1])[69]
        success_kys_all = np.array(get_thresh_success_pair(gts, kys_preds)[1])[69]
        success_tomp_all = np.array(get_thresh_success_pair(gts, tomp_preds)[1])[69]
